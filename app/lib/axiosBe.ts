...
export async function refreshCsrf() {
    ...
    await axios.get('https://api.pinangmajusejahtera.my.id/sanctum/csrf-cookie', {
        withCredentials: true,
    });
    
    // 2. Ambil token yang paling valid langsung dari body response (bukan dari document.cookie)
    try {
        const res = await axios.get('https://api.pinangmajusejahtera.my.id/api/web/csrf-token', {
            withCredentials: true,
            headers: {
                'Accept': 'application/json'
            }
